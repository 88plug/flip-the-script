...
#   FLIP_THE_SCRIPT_PYTHON  this plugin
#
...
find_python() {
  ...
  for c in "${EIGHTYEIGHT_PYTHON:-}" "${PLUGIN_PYTHON:-}" "${FLIP_THE_SCRIPT_PYTHON:-}"; do
    _try "$c" && return 0
  done
...
PY="$(find_python)" || {
  echo "flip-the-script: no Python >=${MIN_MAJOR}.${MIN_MINOR} found." >&2
  echo "  Set EIGHTYEIGHT_PYTHON=/path/to/python3 (or FLIP_THE_SCRIPT_PYTHON), or install Python 3." >&2
